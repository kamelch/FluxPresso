...
public:
     ...
     */
    T update(T value) {
        // Guard: reject extreme values that would poison the running sum.
        // E.g. thermocouple failure (999°C) or HX711 timeout (-999g) would
        // average with good readings to produce misleading intermediate values
        // (e.g. 5×25°C + 999°C = 187°C → false OVERTEMP).
        // Return the last good average instead.
        if (!isfinite((float)value) || (float)value > 900.0f || (float)value < -900.0f) {
            return (count_ > 0) ? (sum_ / count_) : value;
        }
