...
namespace {
    ...
    // Best-effort retention across reset (RTC memory).
    RTC_DATA_ATTR FaultCode g_last_fault_rtc = FAULT_NONE;
    RTC_DATA_ATTR uint32_t g_last_fault_ms_rtc = 0;
    ...
    void push(LogLevel lvl, FaultCode fc, const char* msg) {
        ...
        if (lvl == LOG_FAULT && fc != FAULT_NONE) {
            g_last_fault_rtc = fc;
            g_last_fault_ms_rtc = e.ms;
        }
...
namespace Diag {
    ...
    FaultCode last_fault() { return g_last_fault_rtc; }
    uint32_t last_fault_ms() { return g_last_fault_ms_rtc; }
}
