...
public:
    ...
    // Backwards-compatible aliases (allows using DreamSteamController::MODE_* in code)
    static constexpr Mode MODE_DISABLED      = Mode::MODE_DISABLED;
    static constexpr Mode MODE_CONSERVATIVE  = Mode::MODE_CONSERVATIVE;
    static constexpr Mode MODE_AGGRESSIVE    = Mode::MODE_AGGRESSIVE;
    static constexpr Mode MODE_ADAPTIVE      = Mode::MODE_ADAPTIVE;
    ...
    struct Config {
        ...
        uint8_t pulse_power_min = 40;            // Minimum pump power (%)
        uint8_t pulse_power_max = 60;            // Maximum pump power (%)
        uint16_t pulse_duration_ms = 1000;        // Individual pulse length
        uint16_t pulse_interval_ms = 2000;       // Minimum time between pulses
        ...
        float min_temp_for_refill_c = 140.0f;    // Don't refill if too cold
        float max_temp_for_refill_c = 160.0f;    // Don't refill if too hot
        uint16_t max_continuous_pulses = 8;      // Prevent runaway
        
        // Optional steam valve detection
        bool use_steam_valve_switch = true;
        ...
        , pre_pulse_temp_(0.0f)
    {
        ...
        // Use a sentinel for "invalid" timestamps so that a real sample at t=0ms
        // is treated as valid (important for deterministic native tests).
        for (int i = 0; i < TEMP_HISTORY_SIZE; ++i) {
            temp_history_[i] = 0.0f;
            temp_history_time_[i] = 0xFFFFFFFFu;
        }
    }
...
private:
    ...
    float calculateTempDropRate(uint32_t now_ms) {
        // Find oldest valid entry. We use 0xFFFFFFFFu as "invalid".
        int oldest_idx = temp_history_idx_;
        for (int i = 0; i < TEMP_HISTORY_SIZE; ++i) {
            if (temp_history_time_[oldest_idx] == 0xFFFFFFFFu) {
    ...
    bool canRefill(float temp_c, bool zc_ok, bool steam_valve_open, uint32_t now_ms) {
        ...
        // NOTE: We used to hard-block refill when ZC health was reported bad.
        // In practice, the ZC "health" check can go false under EMI (heater/steam)
        // even while edges are still present and the dimmer can operate.
        // Allow refill attempts; if ZC is truly missing, the dimmer won't fire anyway.
